...
public:
    ...
    void initmysql_result(connection_pool *connPool); // 同步线程初始化数据库
...
private:
    ...
    // m_start_line是已经解析的字符，也是新行在buffer中的起始位置，将该位置后面的数据赋给text
    // get_line用于将指针向后偏移，指向未处理的字符
    //此时从状态机已提前将一行的末尾字符\r\n变为\0\0，所以text可以直接取出完整的行进行解析
    char *get_line() { return m_read_buf + m_start_line; }; // 从状态机读取一行，分析是请求报文的哪一部分
